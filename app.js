...
// ==========================================
const ExportService = {
    async generatePdf(elementId, filename = "Facility_Assessment_Snapshot.pdf") {
    ...
    async generateWordDoc(dataModel, overridenName) {
        try {
            ...
            // Header
            addRow("Name of Facility", finalName);
            addRow("Location", dataModel.location);
            addRow("EMR", document.getElementById("emrInput").value.trim() || "Not specified");
            addRow("Census Capacity", dataModel.capacity);
            addRow("Current Census", document.getElementById("censusInput").value.trim() || "Not specified");
            addRow("Type of Patient", document.getElementById("patientTypeInput").value.trim() || "Not specified");
            addRow("Previous Coverage from Medelite", document.getElementById("prevCoverageInput").value || "Not specified");
            addRow("Previous Provider Performance", document.getElementById("prevPerformanceInput").value.trim() || "Not specified");
            addRow("Medical Coverage", document.getElementById("medCoverageInput").value.trim() || "Not specified");

            // Ratings
            addRow("Overall Star Rating", `${dataModel.ratings.overall} / 5`);
            addRow("Health Inspection", `${dataModel.ratings.health} / 5`);
            addRow("Staffing", `${dataModel.ratings.staffing} / 5`);
            addRow("Quality of Resident Care", `${dataModel.ratings.quality} / 5`);

            // Metrics
            dataModel.metrics.forEach(m =>
                addRow(m.label, m.val || "N/A")
            );
